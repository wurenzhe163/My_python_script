...
def checkpoint(config, epoch, iter, gen, dis):
    model_dir = os.path.join(config.out_dir, 'models')
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
        
    net_gen_model_out_path = os.path.join(model_dir, 'gen_model_epoch_{}_iter_{}.pth'.format(epoch,iter))
    net_dis_model_out_path = os.path.join(model_dir, 'dis_model_epoch_{}_iter_{}.pth'.format(epoch,iter))
