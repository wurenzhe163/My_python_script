...
def _upsample_like(src,tar):
    ...
    src = nn.functional.interpolate(src,size=tar,mode='bilinear',align_corners=True)
...
class REBNCONV(nn.Module):
    def __init__(self,in_ch=3,out_ch=3,
        ...
        self.conv_s1 = nn.Conv2d(in_ch,out_ch,kernel_size,padding=1*dirate,dilation=1*dirate)
        self.bn_s1 = nn.BatchNorm2d(out_ch)

        self.conv_b1 = nn.Conv2d(in_ch,out_ch,1,padding=0,dilation=1*dirate)
        self.bn_b1 = nn.BatchNorm2d(out_ch)

        self.relu_s1 = nn.ReLU(inplace=True)
        self.conv_s2 = nn.Conv2d(out_ch, out_ch, kernel_size, padding=2 * dirate, dilation=2 * dirate)
    ...
    def forward(self,x):
        ...
        xout = self.bn_s2(self.conv_s2(self.relu_s1(self.bn_s1(self.conv_s1(x)))))
...
class classifer(nn.Module):
    def __init__(self,in_ch=1024,out_ch=1):
        ...
        self.Conv_out = nn.Conv2d(in_ch//16,out_ch,3,padding=1,dilation=1)
...
#for i in model.classifer_out[0].parameters():
#    i.requires_grad = False

# model_ft = models.resnet50(pretrained=True)
# # 查看模型参数，二选一即可
# summary(model_ft,input_size=(3,512,512))
# stat(model_ft,(3,512,512))
# # 查看重写前的输入
# num_fc_ftr = model_ft.layer4[0].conv1
