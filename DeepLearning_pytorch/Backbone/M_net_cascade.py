...
class M_net(nn.Module):
    ...
    def __init__(self, mInputChannels=4):
        ...
        #
        # self.conv2 = nn.Conv2d(18, 1, 5, 1, 2)
        # 加入S激活函数
        self.conv1 = nn.Sequential(nn.Conv2d(20, 1, 5, 1, 2),
                                   nn.Sigmoid())

        #         self.conv2 = nn.Sequential(nn.Conv2d(18, 1, 5, 1, 2),
        #                                    nn.Sigmoid())
        # 尝试不使用激活，使用图像精化
        self.conv2 = nn.Conv2d(18, 1, 5, 1, 2)
    ...
    def forward(self, input):
        ...
        # input是train与trimap结合，这里去掉了trimap,转为添加out1
        out2 = self.conv2(torch.cat((y3_3, input[:, 0:1, :, :], out1), 1))
        #         out2 = (1-out1) * input[:,0:1,:,:] + out2
